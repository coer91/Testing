...
namespace HWMX.DotNet
{
    ...
    { 
        ...
        protected string _dtoOutput = $"../Microservices";
        protected string _mapperOutput = $"../Microservices";
        protected string _serviceInterfaceOutput = $"../Microservices";
        protected string _serviceOutput = $"../Microservices";
        protected string _controllerOutput = $"../API";
        protected string _testOutput;
        protected string _serviceCollectionPath = $"../Setup";
        protected string _xunitSetupPath;

        #region EF Core
        protected string _efConnectionString = "";
        protected string _efProvider = "Microsoft.EntityFrameworkCore.SqlServer";
        protected string _efStartupProject = "API";
        protected string _efProject = "Repositories";
        protected string _efNamespace = "";
        protected string _efOutputFiles = "";
        protected string _efContextNamespace = "";
        protected string _efContextOutput = "";
        protected string _efContextName = "";
        #endregion
        ...
        protected void SelectDatabase()
        {
            while (true)
            {
                ...
                foreach (string db in _databaseList)
                {
                    Console.WriteLine($" => {db.ToPascalCase()}");
                }
                ...
                {
                    Alert("Select one database...");
                    continue;
                }

                database = database.ToPascalCase();

                if (_databaseList.Any(x => x.Equals(database)))
                {
                    _database = database;
                    _consoleMessage += $"\nDatabase: {database}";
                    break;
                ...
                Alert($"DbSet<{database}> Not Found...");
            }
        }


        //Scaffold-DbContext Name=HWMXCore -Provider Microsoft.EntityFrameworkCore.SqlServer -StartupProject API -Project Repositories -Namespace Repositories.HWMXCore.Database -OutputDir HWMXCore/Database -ContextNamespace Repositories.HWMXCore.Database  -ContextDir HWMXCore/Database  -Context HWMXCoreContext  -NoOnConfiguring -ErrorAction SilentlyContinue -Force
        protected async Task SyncDatabase()
        {
            ...
            if (Confirm("\nDo you want to sync the database"))
            {
                ...
                if (string.IsNullOrWhiteSpace(_efConnectionString))
                    _efConnectionString = $"Name={_database}";

                if (string.IsNullOrWhiteSpace(_efNamespace))
                    _efNamespace = $"{_efProject}.{_database}.Database";

                if (string.IsNullOrWhiteSpace(_efOutputFiles))
                    _efOutputFiles = $"{_database}/Database";

                if (string.IsNullOrWhiteSpace(_efContextNamespace))
                    _efContextNamespace = _efNamespace;

                if (string.IsNullOrWhiteSpace(_efContextOutput))
                    _efContextOutput = _efOutputFiles;

                if (string.IsNullOrWhiteSpace(_efContextName))
                    _efContextName = $"{_database}Context";
                ...
                using Process process = new()
                {
                    StartInfo = new()
                    {
                        ...
                        + $"\"{_efConnectionString}\" "
                        + $"{_efProvider} "
                        + $"--startup-project {_efStartupProject} "
                        + $"--project {_efProject} "
                        + $"--namespace {_efNamespace} "
                        + $"--output-dir {_efOutputFiles} "
                        + $"--context-namespace {_efContextNamespace} "
                        + $"--context-dir {_efContextOutput} "
                        + $"--context {_efContextName} "
                        + "--no-onconfiguring "
                        + "--force",
                        ...
                        CreateNoWindow = true,
                        WorkingDirectory = Files.GetSolutionPath()
                    }
                };
                ...
                {
                    _consoleMessage += output.Contains("Build succeeded") ? " sync" : $"\n\n{output}";
                    Console.Clear();
                    Console.WriteLine(_consoleMessage);
                }
        ...
        private bool ExistsDbSet(string dbSet)
        {
            try
            {
                string path = $"{_contextPath}/{_database}/Database/{_database}Context.cs";
        ...
        protected void CreateIRepository()
        {
            ...
            if (Directory.Exists(_repositoryInterfaceOutput))
            {
                ...
                if (path is not null)
                {
                    ...
                    streamWriter.WriteLine($"using Repositories.{_database}.Database;");
                    streamWriter.WriteLine($"using System.Linq.Expressions;");
                    streamWriter.WriteLine();
                    streamWriter.WriteLine($"namespace Repositories.{_database}.Interfaces");
                    streamWriter.WriteLine($"{{");
            ...
            else Alert($"{_repositoryInterfaceOutput} Not Found");
        }
        ...
        protected void CreateRepository()
        {
            ...
            if (Directory.Exists(_repositoryOutput))
            {
                ...
                if (path is not null)
                {
                    ...
                    streamWriter.WriteLine($"using Repositories.{_database}.Interfaces;");
                    streamWriter.WriteLine($"using Repositories.{_database}.Database;");
                    streamWriter.WriteLine($"using System.Linq.Expressions;");
                    streamWriter.WriteLine();
                    streamWriter.WriteLine($"namespace Repositories.{_database}.Repository");
                    streamWriter.WriteLine("{");
                    streamWriter.WriteLine($"\tpublic class {_class}Repository({_database}Context _context) : I{_class}Repository");
                    streamWriter.WriteLine($"\t{{");
                    streamWriter.WriteLine();
            ...
            else Alert($"{_repositoryOutput} Not Found");
        }


        protected void CreateDto()
        {
            _dtoOutput = $"{_dtoOutput}/DTOs";

            if (Directory.Exists(_dtoOutput))
            {
                ...
                if (path is not null)
                {
                    ...
                    foreach (string modelFile in Directory.GetFiles($"{_contextPath}/{_database}/Database"))
                    {
            ...
            else Alert($"{_dtoOutput} Not Found");
        }


        protected void CreateMapper()
        {
            _mapperOutput = $"{_mapperOutput}/AutoMappers"; 

            if (Directory.Exists(_mapperOutput))
            {
                ...
                if (path is not null)
                {
                    ...
                    streamWriter.WriteLine();
                    streamWriter.WriteLine("namespace Microservices.AutoMappers");
                    streamWriter.WriteLine("{");
            ...
            else Alert($"{_mapperOutput} Not Found");
        }


        protected void CreateIService()
        {
            _serviceInterfaceOutput = $"{_serviceInterfaceOutput}/Interfaces";
                        
            if (Directory.Exists(_serviceInterfaceOutput))
            {
                ...
                if (path is not null)
                {
                    ...
                    streamWriter.WriteLine("using Microservices.DTOs;");
                    streamWriter.WriteLine("using HWMX.DotNet;");
            ...
            else Alert($"{_serviceInterfaceOutput} Not Found");
        }


        protected void CreateService()
        {
            _serviceOutput = $"{_serviceOutput}/Services";
            ...
            else Alert($"{_serviceOutput} Not Found");
        }


        protected void CreateController()
        {
            _controllerOutput = $"{_controllerOutput}/Controllers";

            if (Directory.Exists(_controllerOutput)) 
            {
            ...
            else Alert($"{_controllerOutput} Not Found");
        }
        ...
        protected void CreateTests()
        {
            if (Directory.Exists(_testOutput))
            {
                ...
                if (path is not null)
                {
                    ...
                    //Class
                    streamWriter.WriteLine($"\tpublic class {_class}Test : Xunit{_class}");
                    streamWriter.WriteLine("\t{");
                    streamWriter.WriteLine();
            ...
            else Alert($"{_testOutput} Not Found");
        }
                ...
                try
                {
                    if (collection.Equals(BUILD.Tests))
                    {
                        ...
                        if (File.Exists(path))
                        {
                            ...
                            if (!dictionary.Any(x => x.Value.Trim().Contains($"class Xunit{_class} : Xunit{_database}Tests")))
                            {
                                double keyRepository = dictionary.OrderBy(x => x.Key).LastOrDefault().Key;
                                dictionary.Remove(keyRepository);
                                dictionary.Add(++keyRepository, $"\n");
                                dictionary.Add(++keyRepository, $"\tpublic abstract class Xunit{_class} : Xunit{_database}Tests");
                                dictionary.Add(++keyRepository, $"\t{{");
                                dictionary.Add(++keyRepository, $"\t\tprotected readonly I{_class}Repository _repository;");
                                dictionary.Add(++keyRepository, $"\t\tprotected readonly I{_class}Service _service;");
                                dictionary.Add(++keyRepository, $"");
                                dictionary.Add(++keyRepository, $"\t\tpublic Xunit{_class}()");
                                dictionary.Add(++keyRepository, $"\t\t{{");
                                dictionary.Add(++keyRepository, $"\t\t\t_repository = new {_class}Repository(_context);");
                                dictionary.Add(++keyRepository, $"\t\t\t_service = new {_class}Service(_repository, _mapper);");
                                dictionary.Add(++keyRepository, $"\t\t}}");
                                dictionary.Add(++keyRepository, $"\t}}");
                                dictionary.Add(++keyRepository, $"}}");
                    ...
                    {                        
                        string prefix = collection.Equals(BUILD.Service) ? "Microservices" : _database;
                        string path = $"{_serviceCollectionPath}/ServiceCollection/{prefix}Collection.cs";
