...
GO
/*
The type for column Id in table [dbo].[TblLanguages] is currently  VARCHAR (10) NOT NULL but is being changed to  INT NOT NULL. Data loss could occur and deployment may fail if the column contains data that is incompatible with type  INT NOT NULL.

The type for column Name in table [dbo].[TblLanguages] is currently  NVARCHAR (50) NOT NULL but is being changed to  VARCHAR (10) NOT NULL. Data loss could occur and deployment may fail if the column contains data that is incompatible with type  VARCHAR (10) NOT NULL.
*/

IF EXISTS (select top 1 1 from [dbo].[TblLanguages])
...
GO
PRINT N'Starting rebuilding table [dbo].[TblLanguages]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_TblLanguages] (
    [Id]   INT          NOT NULL,
    [Name] VARCHAR (10) NOT NULL,
    CONSTRAINT [tmp_ms_xx_constraint_PK_TblLanguages_Id1] PRIMARY KEY CLUSTERED ([Id] ASC),
    CONSTRAINT [tmp_ms_xx_constraint_UN_TblLanguages_Name1] UNIQUE NONCLUSTERED ([Name] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[TblLanguages])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_TblLanguages] ([Id], [Name])
        SELECT   [Id],
                 [Name]
        FROM     [dbo].[TblLanguages]
        ORDER BY [Id] ASC;
...
DROP TABLE [dbo].[TblLanguages];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_TblLanguages]', N'TblLanguages';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_TblLanguages_Id1]', N'PK_TblLanguages_Id', N'OBJECT';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_UN_TblLanguages_Name1]', N'UN_TblLanguages_Name', N'OBJECT';
