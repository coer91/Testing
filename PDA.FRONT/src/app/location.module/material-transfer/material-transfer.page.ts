...
export class MaterialTransferPage extends PagePDA {  
    ...
    protected readonly fifoList    = signal<IMaterialFIFO[]>([]);
    ...
    protected async GetLot(lotNumber: string) { 
        ...
        else if(this.dataSource().some(item => item.Detail.some(itemDetail => itemDetail.LotNumber.equals(lotNumber)))) {
            return null;
        }            
    ...
    protected indicator = computed(() => 
        `${this.dataSource().filter(item => item.Qty == item.QtyChecked).length} / ${this.dataSource().length}`
    );
