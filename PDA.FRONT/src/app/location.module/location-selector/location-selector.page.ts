...
@Component({
    selector: 'location-selector-page',
    templateUrl: './location-selector.page.html', 
    standalone: false
})
export class LocationSelectorPage extends PagePDA {  
    ...
    protected readonly storageList = signal<IStore[]>(this.locationSelectorService.GetStorageList());  
    ...
    protected async GetLocation(scanner: string) {   
        ...
        //Get CaseLabel
            ...
            const lotList = await this.locationSelectorService.GetCaseLabelLocation(lotNumber);
    ...
    protected async Check(scanner: string) {
        ...
        if(lot) { 
            ...
            const response = await this.locationSelectorService.PartNumberLocationMatching(Location, PartNumber);            
    ...
    protected async Save() { 
        ...
        if(aswer) {
            ...
            const response = await this.locationSelectorService.SetLotLocation(storageCode, location, lotList);
