import { StocktakingWarehouseService } from './stocktaking-warehouse.service';
...
@Component({
    selector: 'stocktaking-warehouse-page',
    templateUrl: './stocktaking-warehouse.page.html', 
    standalone: false
})
export class StocktakingWarehousePage extends PagePDA {  
 
    constructor() { super('MM_LT0401') }   

    //Inject   
    private masterService = inject(MasterService);  
    private stocktakingWarehouseService = inject(StocktakingWarehouseService);  
    ...
    protected async GetInspNumberList() { 
        ...
        if(this.storage()) {
            ...
            const response = await this.stocktakingWarehouseService.GetInspNumberList(storageCode);
        ...
        if(!this.dataSource().find(x => x.LotNumber.equals(lotNumber))) {
            ...
            if(lot) {
                if(!lot.StorageCode.equals(this.storage()!.Code)) {
                    ...
                    const response = await this.stocktakingWarehouseService.MoveLot(lot.LotNumber, this.storage()!.Code); 
    ...
    protected async Save() { 
        ...
        if(aswer) {
            ...
            const response = await this.stocktakingWarehouseService.SetInspection(storageCode, inspection, lotList);
